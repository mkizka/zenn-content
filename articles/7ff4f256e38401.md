...
これは↓と結局同じ結果になりますが、一つのプラグインとルールが一つのオブジェクトにまとまっていて見やすいです(感想)。
...
`FlatESLint`クラスは`eslint/use-at-your-own-risk`に入っています。自分がが試した際は型定義がありませんでしたが、importして使用できました。
...
## ESLintプラグインはFlatCompat無しで使える場合もある

eslintのライブラリにはいくつか種類があり、extendsすることが前提になっているeslint-config-xxx系のライブラリはFlat Configに対応していなければ`FlatCompat`の`extends`メソッドを使用する必要があります。
...
ライブラリのdefaultエクスポートがFlat Configとしても問題ない形になっているためと推測できますが、とはいえ他に問題があるかもしれないので良い方法ではないと思います。
